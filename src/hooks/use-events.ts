...
/**
 * Floating-local (Tate 2026-05-25 + 2026-05-26): event.date_start /
 ...
 * so "is past" lines up with what the viewer's phone says. `now` is
 * injectable so unit tests can pass a fixed Date without faking
 * system time or guessing the runner's host tz.
 */
...
export function useMyEvents(tab: 'upcoming' | 'invited' | 'past') {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // event doesn't linger in "upcoming" until midnight UTC.
