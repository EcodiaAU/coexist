    ...
    <BottomSheet open={open} onClose={onClose} snapPoints={[0.85]}>
      <div className="space-y-5 pb-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-neutral-900">Issue a ticket</h2>
        </div>
        ...
        <div className="space-y-2">
          ...
          {searchLoading && <p className="text-xs text-neutral-400 px-1">Searching...</p>}
          {searchResults.length > 0 && (
            <div className="space-y-1 max-h-[220px] overflow-y-auto">
              {searchResults.map((u) => (
                ...
                >
                  <Avatar src={u.avatar_url} name={u.display_name ?? u.email ?? '?'} size="sm" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-neutral-800 truncate">{u.display_name ?? 'Member'}</p>
                    {u.email && <p className="text-[11px] text-neutral-400 truncate">{u.email}</p>}
                  </div>
                  <Send size={15} className="text-primary-500 shrink-0" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="space-y-2 border-t border-neutral-100 pt-4">
          <p className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">Or add by email</p>
          <input {...nameProps} value={name} placeholder="Name (optional)" className={inputCls} />
          <input
            {...emailProps}
            value={email}
            type="email"
